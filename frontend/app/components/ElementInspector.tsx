...
export default function ElementInspector() {
  ...
  const loadUrl = async () => {
    ...
    setError('')
    try {
      const response = await axios.post('http://localhost:5000/api/proxy', { url })
      setProxyHtml(response.data.html)
    } catch (error) {
      console.error('Error loading URL:', error)
      setError('An error occurred while loading the URL. Please check the console for more details.')
    }
    setLoading(false)
  }
  ...
  const identifyElements = () => {
    if (iframeRef.current) {
      try {
        ...
        setProgress(0)
        setLoading(true)
        iframeRef.current.contentWindow.identifyElements()
      } catch (error) {
  ...
  const downloadCsv = async () => {
    try {
      const response = await axios.post('http://localhost:5000/api/generate-csv', 
      ...
      setError('An error occurred while downloading the CSV. Please check the console for more details.')
    }
  }
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="bg-white bg-opacity-10 backdrop-filter backdrop-blur-lg rounded-lg p-6 shadow-xl">
        ...
        {proxyHtml && (
          ...
          >
            {loading ? `Identifying Elements (${Math.round(progress * 100)}%)` : 'Identify Elements'}
          </button>
        )}
      </div>
            ...
            <table className="w-full text-sm text-left text-gray-300">
              <thead className="text-xs uppercase bg-gray-700 text-gray-300">
                <tr>
                  ...
                  <th scope="col" className="px-6 py-3">XPath</th>
                </tr>
              </thead>
              <tbody>
